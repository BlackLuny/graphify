---
name: graphify-query
description: Queries an existing graphify knowledge graph.
...
Before running any subcommand below (`query`), check that `.graphify_python` exists. If it's missing (e.g. user deleted `graphify-out/`), re-resolve the interpreter first:
